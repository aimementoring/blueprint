...
const calculateCountdown = (endDate, startDate = new Date().toISOString()) => {
  let diff = (momentTimezone$1.tz(endDate, 'America/New_York') - momentTimezone$1.tz(startDate, 'America/New_York')) / 1000; // clear countdown when date is reached
...
const CountdownTimer = ({
  ...
  const _useState = react_7([0, 0, 0, 0, 0]),
  ...
  }, countDown.map((time, index) => react.createElement(TimeColumn, {
    time: time,
...
CountdownTimer.defaultProps = _objectSpread2({}, defaultComponentPropTypes, {
