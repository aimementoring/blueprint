...
function request(url, options) {
  var fetchOptions = _objectSpread({}, options);

  if (options.method === 'POST' || options.method === 'PUT') {
    fetchOptions.headers = _objectSpread({
      Accept: 'application/json',
      'Content-Type': 'application/json; charset=utf-8'
    }, options.headers);

    if (options.hasAuthorization) {
      fetchOptions.headers.Authorization = "Bearer ".concat(getFromStorage('loginToken'));
  ...
  return browserPonyfill(url, fetchOptions).then(checkStatus).then(function (response) {
