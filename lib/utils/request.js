...
function request(url, options) {
  var fetchOptions = _objectSpread2({}, options);

  if (options.method === 'POST' || options.method === 'PUT') {
    fetchOptions.headers = _objectSpread2({
  ...
  return nodePonyfill(url, fetchOptions).then(checkStatus).then(function (response) {
